  ...
  <AuthLayout 
    ...
    <el-form 
      ...
      <div class="auth-footer">
        <p>还没有账户？</p>
        <el-button link @click="$router.push('/register')" class="link-btn">立即注册</el-button>
      </div>
    </el-form>
    <template #features>
...
export default {
  name: 'Login',
  components: { User, Lock, Key, CircleCheck, StarFilled, AuthLayout },
  setup() {
    ...
    const onSubmit = async () => {
      ...
      try {
        ...
        let display = '登录失败，请稍后重试'
        if (raw.includes('密码') || lower.includes('password')) display = '密码错误'
        else if (raw.includes('用户名不存在') || lower.includes('user not found') || lower.includes('unknown user')) display = '用户名不存在'
